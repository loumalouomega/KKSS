...
Use app__check_simulation_environment to check manual and assistant runtimes independently. Use app__study_* for optional studies, app__run_review / app__run_review_export for terminal runs, and app__run_quantity_evaluate to save an explicitly defined field quantity with its unit and source revision. Queue preview never launches work; app__queue_enqueue persists a paused plan and app__queue_resume runs its exact approved revision, while app__queue_resume_row resumes one selected waiting run and keeps other rows held. Use app__queue_retry_variant_preview for a failed row; it creates a fresh run identity after the concrete preview is approved and enqueued. Inspect, pause and cancel through app__queue_* tools. CAD queue mesh exports carry owner-scoped receipts; use cad__job_status to reconcile and cad__job_cancel to cancel the exact receipt owner/request, and leave uncertain receipts unretried. Variant comparisons identify mesh-sensitivity, solver-parameter and mixed studies separately. Use app__variants_preview / app__variants_create for explicit variant tables and app__variants_compare / app__variants_compare_export to inspect or export a comparison. Study duplication never copies active processes or result ownership. \
