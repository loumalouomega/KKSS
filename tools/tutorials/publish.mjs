...
for (const c of cases) {
  ...
  const verification = JSON.parse(execFileSync(python, [path.join(root, 'tools/tutorials/verify.py'), c.id, source], { encoding: 'utf8' }));
  const dest = path.join(output, c.id);
  await fs.rm(dest, { recursive: true, force: true });
  await fs.mkdir(dest);
  await fs.copyFile(path.join(root, 'LICENSE'), path.join(dest, 'LICENSE'));
  for (const name of await fs.readdir(source)) {
    if (name === 'vtk_output' || name === c.geometry || name.startsWith(c.geometry + '.') && !name.endsWith('.geo') ||
  ...
  verification.revisions = Object.fromEntries(['cad', 'mesh'].map(name => [name, execFileSync('git', ['-C', path.join(root, name), 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim()]));
