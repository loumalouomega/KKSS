...
| 🔶 **Post-Processing** | [VSCode-MDPA-Preview](https://github.com/loumalouomega/VSCode-MDPA-Preview) (`mesh/`) | MDPA/VTK inspection plus 44 extended mesh formats via meshio++ (Gmsh, Abaqus, Nastran, UNV, SU2, Exodus, CGNS, MED, GiD postprocess, EnSight Gold, Triangle, OpenFOAM, …), split view with per-pane fields and cameras, combinable field modes (contour/isosurface/quiver/deformed shape) & time-series visualization, beam/sphere rendering and face-normal (inverted-element) checks, mesh quality, mesh-size, watertightness and field-integral analysis, a data table with CSV/XLSX export and field-value plots over time, 30+ mesh operations with undo/redo and a batch queue (smoothing, renumbering, partitioning, refinement, cropping, field calculator/gradient/Hessian, error estimation, signed distance, field transfer, merging, MMG remeshing with expression-driven and anisotropic sizing, frozen entities and per-part size bounds), multi-point constraints maintained through every edit, Kratos case setup, tracked solver runs and viewport recording via built-in problemtypes (incl. the Flowgraph node-editor) |
