...
The app opens on a **home screen** with one button per task; a toolbar toggle (and `Ctrl+0` for Home) switches between the screens, and both viewers stay alive, keeping their loaded file, camera, and history. Home also checks the manual and assistant simulation environments independently and can track optional project studies under `.kkss/project.json`; see [guided simulation work](doc/guide/getting-started.md#guided-simulation-work). Also in the box:
