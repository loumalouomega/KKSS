...
The mesh submodule's **Flowgraph** problemtype (`view: "flowgraph"`) splits the MDPA preview's viewport to embed the AGPL-3.0-or-later [`@kratos-flowgraph/flowgraph`](https://www.npmjs.com/package/@kratos-flowgraph/flowgraph) node editor in an `<iframe>`, served by a small Express+EJS app the submodule forks on demand (`mesh/src/flowgraphServer.ts`, `mesh/src/flowgraphController.ts`). `app/main/index.ts` owns the shared, ref-counted `FlowgraphController` instance — mirroring `mesh/src/extension.ts`'s `activate()` — and injects it into every `MeshHost`, which passes it into `new MdpaEditorProvider(context, flowgraph, runs, recents)` (the VTK provider also takes `(context, flowgraph, runs, recents)` and owns a Problemtype controller for non-MDPA formats) and disposes it on Electron's `will-quit` so the forked child process never outlives the app. `runs` is the shared `RunManager` and `recents` mesh 3.15.0's `RecentMeshStore` — the same construct-once-inject-everywhere rule, for the same reason: a solve, and the recent-file list, both outlive the tab they came from.
...
Native menu enabled states are rebuilt after every screen switch, including file-open routing. Rebuilding only when recent files changed left mesh actions disabled on the first mesh open because that notification preceded the mode change. The smoke suite checks packing availability after file open and CAD/mesh switches.

### Submodule release maintenance
...
5. Start the **copied** CAD and mesh MCP servers with the environment from `serverSpecs()` in `mcpManager.ts`, list their actual tools and compare both missing and obsolete names against `toolPolicy.ts`. Classify new tools deliberately; the pinned test checks the policy table, not the live upstream tool list. Verify new viewer capabilities through MCP too, and update the stable system-prompt capability description for new app abilities. Settings and current-file context must stay aligned where applicable.
6. Run `npm run typecheck`, `npm test`, `npm run smoke` (under `xvfb-run -a` on headless Linux), and `npm run docs:build`. Exercise the release's new operations with upstream fixtures and temporary outputs. Update user guides, README and project memory; regenerate screenshots with `npm run docs:screenshots` if viewer markup or visible behavior changes.

The CAD MCP kernel is built from the unchanged `cad/src/kernelWorker.ts` by `cadMcpWorkerConfig` in `esbuild.mjs`. It shares the interactive worker's Gmsh CJS alias and meshio/fTetWild loaders, but lives at `out/cad-runtime/dist/kernel-worker.js`, beside OCCT/Gmsh WASM. The loaders also resolve `../../meshio` and `../../ftetwild` from there. **Do not overwrite it with `cad/dist/kernel-worker.js`**: that upstream artifact expects external npm packages shipped by the extension, which KKSS does not include. `test/cadMcpRuntime.test.ts` checks real OCCT and meshio calls through the bundled server and forked worker.

Format counts depend on whether extensions, reader keys or writer keys are counted. Use `mesh/src/parser/meshioFormats.ts`'s read/write routing tables and `mesh/src/parser/writers/exportFormats.ts` for the supported surfaces instead of a shared approximate count. Packed time series use an `.xdmf` index plus a required sibling `.h5`; keep both when copying results.
...
In streamed deployments, `app/main/services/managedConfig.ts` overlays operator
values in memory above `stateStore`: `KKSS_LLM_PROVIDER`, model/base URL,
secret-file, project root, restore, theme/zoom, meta-server and cloud client
settings are read at startup, shown as managed in the native menu, and blocked
from persistence. `app/main/services/headless.ts` exposes only loopback,
bearer-protected activity/shutdown endpoints. The SIGTERM path saves dirty mesh
operation recipes and editor buffers (with recovery files where possible), then
drains cloud uploads before the normal synchronous store flush. Unknown or stale
job state inhibits the container's idle timer.

`ChatService` runs the agent loop: a provider adapter streams one model turn, tool calls are dispatched, and the loop repeats until the model stops calling tools (or the user hits Stop — dangling tool calls are pruned from the next request by `transcript.ts`). API providers live under `app/main/services/chat/providers/`: **Anthropic** uses `@anthropic-ai/sdk`, while **OpenAI-compatible** uses raw `fetch` + SSE. Subscription providers live under `app/main/services/chat/agents/`: **Codex** speaks the official Codex App Server protocol over stdio, and **Claude Code** uses the official Agent SDK with an in-process MCP server. Both subscription adapters call the same `ChatService.executeTool()` dispatcher, so approval, denial, dry-run, image, cancellation, and transcript behavior is shared. Subscription sessions persist an opaque provider session id and a hash of the KKSS tool catalog; a mismatch starts a fresh session seeded with transcript data. Historical tool calls are sent as context and are never executed again.
...
`kratosRuntime.ts` probes the app-local uv executable, PATH's `uvx`, then PATH's `uv`. Discovery never installs anything. The fixed-purpose `installKratosRuntime` and `retryKratos` chat IPC actions reach the shared hub's Kratos-only reconnect operation; they accept no paths, commands, or URLs. Status carries an optional startup phase (`probing`, `installing`, `preparing`) and a failure category, with bounded diagnostics. The assistant sees current availability through the context suffix, while its tool definitions remain fixed for each user turn.

The official uv `0.12.10` installer runs with `UV_UNMANAGED_INSTALL` under `<userData>/runtimes/`, verifies the executable, and promotes it to `<userData>/runtimes/uv`. Temporary files are removed on failure and success. Existing system installations and uv cache/Python locations are preserved. Overlapping operations share one promise; shutdown aborts setup and closes pending MCP transports. Kratos connection and tool discovery each allow five minutes; CAD/mesh retain their existing connection timeout and tool-call timeouts are unchanged.

The `mcp<2` constraint is retained for `kratos-mcp-server@0.5.0`, which still imports `mcp.server.fastmcp`. The version pin is shared by the launcher, bundled-runtime validation, and offline environment probes; Docker requirements and their hash lock must be updated alongside it. The published 0.5.0 package was verified to initialize and advertise 52 tools, 13 resources, and five prompts. Linux installer and Electron recovery are verified; Windows and macOS installer commands have unit coverage but remain unverified on real machines. `node tools/kratosStartup.e2e.mjs` runs the isolated-profile recovery check, also used by `npm run docs:screenshots`.
...
`unclassifiedTools()` is the submodule-bump seam: `ChatService` logs the unclassified names once per process, and `test/chatToolPolicy.test.ts` pins the exact 83-name policy key set. Compare it with live `tools/list` during a bump: the snapshot alone cannot discover new upstream tools.

#### Validate (dry run)

Four cad tools take a `dryRun` parameter (`cad__apply_edit_ops`, `cad__run_parametric_script`, `cad__run_saved_script`, `cad__import_svg`), listed in `toolPolicy.ts`'s `DRY_RUN_PARAM`. When the blocked call is one of them the prompt grows a fourth button, **Validate (dry run)**, which re-issues it with that flag set so the user can answer the prompt with a report in front of them.
...
- **`cad-preview-macros.json` and `cad-preview-mesh-presets.json` are never synced** — both are
  per-*folder* libraries while the cache is per-*file*, so uploading either would let two models
  from one remote folder overwrite each other's.
...
All settings are defined in **one registry**, `app/main/services/settings/registry.ts`. It is pure: no Electron and no stateStore. Each entry declares:
- an id (`kratos.pythonPath`, `appearance.uiTheme`, …), label, category, type, default and description;
- the stateStore key that holds it (older keys keep their names, so nothing stored is lost);
- when a change applies;
- optionally, a `vscode: {section, key}` mapping.

Three consumers read it:

- **The shim.** `vscodeShim.ts`'s `workspace.getConfiguration(section).get(key, default)` looks up `section.key`. Sections may be dotted, as in `kratos.flowgraph`. A mapped key is served from the stateStore; anything else returns the caller's default. That is how every `kratos.*` setting the unmodified mesh providers read works, with no submodule change. The shim also implements `update()` and `onDidChangeConfiguration`.
- **The Settings page.** `app/renderer/settings/` runs in its own singleton `BrowserWindow`, owned by `services/settings/settingsWindow.ts`, with `settings:toHost`/`settings:toWebview` guarded by sender.
  - Main sends the schema once and then a `rows` snapshot on every stateStore change.
  - Every write is validated by the registry's `toStored` before it lands. A value equal to the default, or an empty string, deletes the key.
  - Entries with side effects route through the same functions the native menu uses: zoom, the update channel, the MCP server, cloud accounts, and the "Never ask" confirmation (`menu.ts`'s `confirmApprovalOff`).
  - Secrets are write-only. The renderer submits a new value but only ever receives `isSet`.
- **The native menu.** `menu.ts` keeps quick radios built from registry entries (`enumRadio`) and is rebuilt on any change to a registry-owned stateStore key. The page and the menu cannot drift.

`stateStore.onDidChange(key, value)` is the change event behind all three. `test/settingsRegistry.test.ts` parses both submodules' `contributes.configuration` and requires every property to be either a registry entry with the same type, enum and default, or listed in `NOT_APPLICABLE` with a reason. A submodule bump that adds a setting fails that test until KKSS decides what to do with it.

**UI theme.** `services/appearance.ts` (with the pure `appearanceCore.ts`) resolves `uiTheme` through Electron's `nativeTheme` to one of VS Code's four theme kinds (`vscode-dark`, `vscode-light`, `vscode-high-contrast`, `vscode-high-contrast-light`).
- **First paint.** Every preload imports `app/preload/appearance.ts`, which fetches the snapshot synchronously (`kkss:appearance`). It puts the kind as a class plus `data-vscode-theme-kind` on `<html>` and `<body>`, and sets the UI font as CSSOM custom properties.
- **Live changes.** Main re-broadcasts to every `webContents` when a relevant key or the OS theme changes.
- **Palettes.** `vscode-vars.css` has a `:root` (Dark+) block plus one block per other kind. `check-theme-vars.mjs` fails the build if a block does not redefine exactly `:root`'s colour variables.
- **CAD viewer.** It restyles itself: its `viewer.css` keys on the same body classes, and a MutationObserver re-reads the palette.
- **Mesh viewer.** Its *auto* scene samples the body background only when a theme is applied. The view shim therefore re-fires mesh's hidden `#theme-select` change on each appearance change while it is `auto`.
- **Editor and terminal.** They read the same snapshot for their own options: CodeMirror through a `Compartment`, xterm through `term.options`.

**Kratos environment.** `services/settings/kratosEnv.ts` builds the delta that `McpManager` spreads over the Kratos MCP server's environment on every (re)start. It uses mesh's vscode-free `problemtype/kratosEnv.ts`, the same code the Run button uses. `McpManager.restartKratos()` backs the page's restart button.
...
- **`kkss://` and `kkss-file://`** schemes — replacements for `asWebviewUri`/`localResourceRoots` (app assets and allow-listed user files respectively). External `kkss://open?file=…` links are validated by `app/main/launch.ts` and enter the same deferred queue as Finder and second-instance opens. Native associations and Linux MIME entries are checked against the CAD and mesh registries by `npm run check:packaging`.
- **`app/main/cadCompute.worker.ts` / `cadComputeClient.ts`** — the RPC boundary for everything WASM-backed on the cad side. The worker spreads fifteen modules into one flat method map (the submodule's `occtService`, `gmshService`, `massProperties`, `entityFacts`, `meshioService`, `meshioRegionParts` — CPU-only but pulls in Three.js — `hitTestService`, `meshHeal`, `primitiveReport`, `meshRegionFit`, `primitiveWrite`, `svgSilhouetteHost`, `modelDiffHost` and `stepPartsService`, plus KKSS's own `cadBRepCache`), and throws at startup if two export the same name; the client re-exposes each with the submodule's own signature via `Parameters<typeof …>`, so a changed signature is a type error rather than a runtime surprise. Pure text helpers (`stepUnits`/`igesUnits` unit detection) stay in the main process — a worker round trip would buy nothing. Since cad 3.0.0 the client also tracks **kernel readiness** for the status bar: `cadKernelStatus.ts` (Electron-free, so unit-tested) feeds cad's own reducer from `start`/`success`/`failure` events around every call and `reset` when the worker dies, translating KKSS's two differently-spelled RPC names (`loadBRepCachedInWorker`, `releaseBRepCache`) through `KKSS_FN_ALIAS`; `cadHost.ts` fans changes out as `kernelStatus` to every open CAD tab, and also posts `documentInfo` (the menubar chip — see [Pre-Processing mode](/guide/cad-mode#reading-the-window-at-a-glance)).
- **CAD-Preview 3.4 interactive additions** — `CadHost` handles `meshSweepRequest` with the shared `runMeshSweep` loop, document-owned cancellation, optional folder selection and `.msh` writes; `computeHoleTable` is exposed through the existing mass-properties worker module for `holeTableRequest`. The generated webview is sourced from the submodule's current markup and bundle, so these controls require no KKSS page copy.
...
- **`app/renderer/theme/mesh-overrides.css`** — KKSS-only overrides for the mesh webview. It hides `#file-menu` and `#theme-select`: mesh 3.0.0 put the viewer's File menu and scene-theme picker in an in-flow `#menubar` strip, both of which the native menu already owns. The markup is still emitted (`webview/main.ts` looks those nodes up by id), so anything reachable *only* from that menu — Save/Load Problem — needs a native menu entry. The strip itself stays visible since the mesh UI redesign, because it holds the **document chip** (`#doc-chip`: file name, format badge, "N unsaved edits", fed by the `documentInfo` message), and the page now ends in a `#statusbar` (engine activity from `engineStatus`, model counts, timeline frame, last Inspect pick). Both messages ride `MeshHost`'s unfiltered `postMessage` relay — no KKSS-side handling — so a mesh bump only has to keep `meshBody()` mirroring `buildPreviewHtml` (the status bar is `STATUSBAR_HTML`, emitted after `#main`). The engine line reflects only the main process's engines: the chat sidebar's mesh MCP server is a separate child with its own meshio++/MMG, which the chip does not see. Note the converse: mesh 3.14.4's **Edges** toggle lives in the visible `View ▾` popup, not the strip, so it deliberately has no native entry (and no host message that could drive one).
- **`app/renderer/theme/vscode-vars.css`** — the `--vscode-*` theme variables VS Code normally injects; `tools/check-theme-vars.mjs` fails the build if a submodule update uses one that is missing, and likewise for a `--ds-*` token `style.css` uses but `design-system.css` doesn't define. The guard also scans every stylesheet under `app/renderer/`, so a `--vscode-*` variable used by KKSS's own pages must be defined here too.
- **`app/renderer/theme/kkss-ui.css`** + **`app/renderer/glyphs.ts`** — the design layer KKSS's *own* pages (shell, home, editor, terminal, chat, jobs, About, What's New, picker) share, matching CAD-Preview v3.0.0's look. `kkss-ui.css` holds the `--ui-*` metric tokens (radii, spacing, type sizes, control heights — never colour) and the shared recipes: `.btn` (ghost by default; `-secondary`/`-primary`/`-outline`/`-sm`/`-link`), `.icon-btn`, `.tb-div`, `.seg` (segmented control), `.section-header`/`.panel-icon`/`.eyebrow`, `.card`, `.field`, `.ui-badge`/`.ui-dot`, `.toast`, `.spinner`, the global `[hidden]` rule and the `:focus-visible` ring. Every page links it right after `vscode-vars.css` and before its own stylesheet, which therefore holds only layout and page-specific bits (a page that needs a pixel value uses the token scale: 5px controls, 8px cards). `glyphs.ts`'s `glyph(id, size?)` re-exports cad's line-glyph set (`cad/src/uiGlyphs.ts`) plus KKSS extras (`x`, `menu`, `send`, `history`, …); add a missing icon to `KKSS_GLYPHS` there. The generated TikZ `shell/shellIcons.ts` still supplies the mode/home/open/edit/terminal/chat/settings/help icons. Strict-CSP pages (`style-src kkss:`) must not use inline `style=` attributes — toggle the `hidden` attribute instead.
...
## Full Electron acceptance suite

After `npm run build`, run `npm run e2e` (`xvfb-run -a npm run e2e` on headless Linux). CI runs this suite sequentially, without assertion retries, and uploads `test-results/` logs, page URLs, HTML and screenshots on failure. `npm run smoke` remains the shorter viewer/Home check. Individual scenarios live under `tools/e2e/` and can be run directly with Node.

The suite exercises generated CAD/mesh DOM, native menu/shim reload, CAD → mesh export and repeat-export refresh, editor saves, PTY output, tabs and actual view zoom, streamed chat with approval/denial/cancellation and transcript persistence, authenticated HTTP MCP enable/call/disable, cloud upload draining and recovery, session restoration, and real single-instance acquisition/forwarding/reacquisition. Assertions check rendered state, disk contents and service effects. Linux is the acceptance platform; these tests do not claim macOS/Windows validation.
