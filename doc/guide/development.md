...
`unclassifiedTools()` is the submodule-bump seam: `ChatService` logs the unclassified names once per process, and `test/chatToolPolicy.test.ts` pins the exact 72-name policy key set. Compare it with live `tools/list` during a bump: the snapshot alone cannot discover new upstream tools.
