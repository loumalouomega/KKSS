...
| `.msh` `.inp` `.bdf` `.nas` `.fem` `.unv` `.mesh` `.vol` `.su2` `.xdmf` `.xmf` `.off` `.avs` `.dat` `.tec` `.mphtxt` `.node` `.ele` `.f3grid` `.pf3` `.post` `.dato` `.ugrid` `.mfm` `.wkt` `.xml` `.dex` `.ip` `.mff` `.case` `.geo` `.poly` `.e` `.exo` `.ex2` `.cgns` `.h5m` `.hmf` `.med` `.foam` | 🔶 Post-Processing | Extended formats read via [meshio++](https://github.com/nschloe/meshio) (Gmsh, Abaqus/ANSYS, Nastran, I-deas UNV, Medit, Netgen, SU2, XDMF, COMSOL, tetgen, EnSight Gold, Triangle, Exodus II, CGNS, MOAB, Salome MED, OpenFOAM, …). Named groups (Gmsh physical groups, Abaqus `*NSET`/`*ELSET`/`*SURFACE`, Exodus element blocks / node sets / side sets, OpenFOAM boundary patches) arrive as SubModelParts. `.e`/`.exo`/`.ex2` (Exodus) and `.xdmf`/`.xmf` describe their time steps within the dataset and drive the timeline from there (XDMF may reference a required sibling `.h5`; copy it too) — no `<prefix>_<rank>_<step>` sibling naming needed; Salome `.med` also accepts a time step, but reports no step count up front, so its timeline isn't discoverable before a read. `.dex`/`.ip`/`.mff` are field-only formats — they carry point fields with no geometry, so reading one yields a point cloud (or an empty mesh); `.case`/`.geo` (EnSight Gold) and `.poly` (Triangle) need their sibling file(s) alongside. `.foam` is an OpenFOAM case: the file you open is a 0-byte marker, and the real mesh — volume cells as Elements, boundary faces as Conditions, one named SubModelPart per boundary patch — lives in a sibling `constant/polyMesh/` tree, which the preview also watches, so re-running `blockMesh`/`snappyHexMesh` refreshes the view in place. Not read: time-directory fields, zones, moving-mesh cases, multi-region and decomposed (`processor*/`) cases. **Saving a `.foam` case in place is refused** — writing "the file" would rewrite the real mesh tree and collapse every patch name into one generic `defaultFaces`; use Export or Save As to write a new case directory instead |
| `.stl` `.obj` `.ply` | Both | Opens in the **currently active** mode |

Pre-Processing can *also* read `.vtk` `.vtu` `.med` `.cgns` `.exo` `.e` `.xdmf` `.mdpa` `.msh` `.inp` `.unv` `.su2` `.mesh` `.post.msh` `.foam` through meshio++, as a **geometry-only boundary surface** (named regions become Parts; fields are not converted, though a scalar point/cell field can be used to colour the model). Those are post-processing formats here and mesh mode reads them natively — fields, blocks and SubModelParts — so opening one always lands in Post-Processing regardless of the active mode. Reach the CAD-side importer from Pre-Processing's own **Open…** dialog (or drag the file onto the CAD view) when you specifically want to mesh or edit that geometry.

`.msh2` (Gmsh 2.2) goes the other way and opens in **Pre-Processing** whatever the active mode — the mesh engine never registered it at all, so cad mode is the only one that can open it.
