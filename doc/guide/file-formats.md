...
Pre-Processing can *also* read `.vtk` `.vtu` `.med` `.cgns` `.exo` `.e` `.xdmf` `.mdpa` `.msh` `.inp` `.unv` `.su2` `.mesh` `.post.msh` through meshio++, as a **geometry-only boundary surface** (named regions become Parts; fields are not converted, though a scalar point/cell field can be used to colour the model). Those are post-processing formats here and mesh mode reads them natively — fields, blocks and SubModelParts — so opening one always lands in Post-Processing regardless of the active mode. Reach the CAD-side importer from Pre-Processing's own **Open…** dialog (or drag the file onto the CAD view) when you specifically want to mesh or edit that geometry.

Two formats go the other way and open in **Pre-Processing** whatever the active mode: `.foam` (an OpenFOAM case directory — Post-Processing can *write* one but not read it back, since that needs a tree of sibling files) and `.msh2` (Gmsh 2.2, which the mesh engine never registered).
