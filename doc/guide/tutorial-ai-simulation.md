...
7. Ask it to use `mesh__case_evaluate_quantity` on the final VTK file with the run id from `mesh.kratosrun.json`, field `DISPLACEMENT`, kind `Nodal`, component `z`, reduction `min`, time 1 s and unit `mm`. The result should be about −0.000589 mm. Compare with the −0.000600 mm beam reference and the 25% tolerance in the [structural result check](/guide/tutorial-structural#verified-result). Open results in the app, select `DISPLACEMENT` in **Field**, enable **Deformed**, keep **Deform by** on `DISPLACEMENT`, and use **Warp scale** 1000× to see the scaled shape. Select `VON_MISES_STRESS` to inspect stress.
