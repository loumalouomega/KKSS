...
features:
  - icon: 🔷
    ...
    details: Open STEP/IGES/BREP, STL/OBJ/PLY/glTF and OpenSCAD .csg/.scad models — plus VTK/MED/CGNS/Exodus/XDMF/MDPA as boundary surfaces via meshio++ — or start from a blank model, define parts, apply parametric edits with query-pinned operands, measure exactly and read mass properties, annotate the view, and generate finite-element meshes with Gmsh, with quality statistics and unit-converted export — powered by the CAD-Preview engine (Three.js + OpenCascade WASM).
  - icon: 🔶
    title: Post-Processing mode
    details: Inspect MDPA models, VTK results, and 42 extended readable formats via meshio++ (Gmsh, Abaqus, Nastran, UNV, SU2, Exodus, CGNS, MED, GiD postprocess, EnSight Gold, Triangle, OpenFOAM, …) with combinable field modes (contour, isosurfaces, quivers, deformed shape), sphere/particle glyphs and face-normal checks, quality and mesh-size reports, click-to-inspect probing, threshold isolation, oblique clipping, time-series playback, split view with per-pane fields, a data table and time-series plots, tracked Kratos runs and viewport recording, and 30+ mesh operations with undo/redo (smoothing, renumbering, partitioning, refinement, cropping, field calculator, merging, anisotropic and expression-driven MMG remeshing); set up and run Kratos cases via built-in problemtypes, including the Flowgraph node-editor — powered by the VSCode-MDPA-Preview engine (vtk.js + MMG WASM).
  - icon: 🔀
  ...
  - icon: 🤖
    title: AI assistant
    details: A chat sidebar where an LLM (Anthropic Claude or any OpenAI-compatible backend) drives CAD editing, meshing, case setup, and Kratos simulations through the engines' MCP tool servers and the kratos-mcp-server. Conversations are saved and listed, so nothing is lost on quit.
  - icon: 🧰
